...
export class AppService {
  ...
  public async publish() {
    await Promise.all(
      Array.from({ length: 30 }).map(async () => {
        const messageId = await this.googleCloudPubsubPublisher.publish(
          'order.created',
          {
            data: {
              field1: 'field1',
  ...
  @GoogleCloudPubsubBatchSubscribe(
    'order.created',
    'order.created.subscription.order-processor-service',
  )
  ...
  @GoogleCloudPubsubSubscribe(
    ...
    'order.created.dead-letter-queue.subscription',
  )
