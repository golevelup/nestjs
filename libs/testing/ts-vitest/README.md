# Vitest Mocking (ts-vitest)

With `@golevelup/ts-vitest` `createMock` utility function, you can easily generate deeply nested mock objects for unit
...
```bash [npm]
npm install @golevelup/ts-vitest -D
```

```bash [yarn]
yarn add @golevelup/ts-vitest -D
```
